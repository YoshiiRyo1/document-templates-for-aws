...
| 未使用のリソース（停止中の EC2、未アタッチの EBS ボリューム、古い AMI・スナップショット等）を定期的に棚卸しするプロセスはありますか？ | 不要リソースの放置はコスト増加の主要因である。<br />AWS Trusted Advisor のコスト最適化チェックで未使用・低使用率のリソースを特定できる。<br />Cost Optimization Hub で組織全体の廃止推奨事項を一元確認できる。<br />棚卸しの実施頻度、リソース削除の承認フロー、削除前のバックアップ取得ルールを定義する。 |        |
| データ転送コスト（リージョン間・AZ 間・インターネット向け）を設計段階で見積もっていますか？ | データ転送コストは AWS 利用料金の中で見落とされやすい項目である。<br />VPC エンドポイント（Gateway / Interface）で S3・DynamoDB 等へのアクセスに NAT Gateway 経由のコストを回避できる。<br />CloudFront でオリジンからのデータ転送コストを削減できる。<br />同一 AZ 内通信の優先、不要なクロスリージョン転送の排除、VPC フローログによるデータ転送量の可視化を検討する。 |        |
