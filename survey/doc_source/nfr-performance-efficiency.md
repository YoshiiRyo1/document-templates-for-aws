...
| コンテナやサーバーレスの採用方針はありますか？ | ECS / EKS でコンテナ化し、Fargate でサーバーレスに運用できる。<br />App Runner で Web アプリケーションを簡易にデプロイ・スケールできる。<br />Lambda はイベント駆動処理に適しており、Lambda SnapStart で Java の起動時間を短縮できる。 |        |
